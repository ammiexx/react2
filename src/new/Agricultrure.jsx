  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (!response.ok) throw new Error('❌ Failed to fetch agricultural products');
        let data = await response.json();

        // Filter for category "agriculture" and verified = true
        data = data.filter(
          (item) => item.category === 'agriculture' && item.verified === true
        );
  ...
  return (
    <div className="max-w-[1200px] mx-auto my-10 px-5 text-[#2c3e50] font-sans w-full">
      <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">
        🌾 Your agricultural product essentials!
      </h2>
      ...
      <section className="mb-12 w-full">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-500">No agricultural products found.</p>
        ) : (
