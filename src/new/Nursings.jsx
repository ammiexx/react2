  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for category "nursing" and verified = true
        data = data.filter(
          (item) => item.category === 'nursing' && item.verified === true
        );
  ...
  return (
    <div className="max-w-[1200px] mx-auto my-10 px-5 text-[#2c3e50] font-sans w-full">
      ...
      <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">
        🏥 Elderly care centers — find trusted nursing services!
      </h2>
      ...
      <section className="mb-12 w-full">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-500">No elderly centers found.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
            {products.map((item) => (
              <div
