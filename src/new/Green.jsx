...
const Nearby = () => {
  ...
  useEffect(() => {
    const fetchGreenItems = async () => {
      try {
        const response = await fetch(
          `https://djanagobackend-5.onrender.com/api/products/`
        );
        if (!response.ok) throw new Error('Failed to fetch green items');
        let data = await response.json();

        // Filter for category "green" and verified = true
        const filteredData = data.filter(
          (item) => item.category === 'green' && item.verified === true
        );

        setGreenItems(filteredData);
      } catch (err) {
  ...
  return (
    <div className="max-w-[1200px] mx-auto my-10 px-5 text-[#2c3e50] font-sans w-full">
      <label htmlFor="search" className="text-xl font-bold mb-3 text-center">
        🎯 <strong>Sustain your lifestyle — eco-conscious goods for everyday living 🌱💚</strong> 👀 💡
      </label>
      ...
      <section className="mb-12 w-full">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-500">No green services found.</p>
        ) : (
