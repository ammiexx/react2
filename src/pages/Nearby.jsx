...
const Nearby = () => {
  ...
  const handleCheckout = async (product) => {
    ...
    const response = await fetch(
      'https://djanagobackend-5.onrender.com/backend/create-checkout-session/',
      {
        method: 'POST',
            ...
            {filteredProducts.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Title */}
                  <h6 className="text-lg font-bold text-center text-[#2c3e50] mt-2 mb-2">{item.product_name}</h6>
      ...
      {zoomedImage && (
        ...
        >
          <img
            src={zoomedImage}
            alt="Zoomed"
            className="max-w-[90%] max-h-[90%] rounded-lg shadow-lg transform scale-105 transition"
          />
        </div>
      )}
    </div>
