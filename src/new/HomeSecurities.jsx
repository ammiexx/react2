...
const Nearby = () => {
  const [homeSecurityItems, setHomeSecurityItems] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const navigate = useNavigate();

  useEffect(() => {
    const fetchHomeSecurityItems = async () => {
      try {
        const response = await fetch(
          `https://djanagobackend-5.onrender.com/api/products/`
        );
        if (!response.ok) throw new Error('Failed to fetch home security items');
        let data = await response.json();

        // Filter for category "homesecurity" and verified = true
        const filteredData = data.filter(
          (item) => item.category === 'homesecurity' && item.verified === true
        );

        setHomeSecurityItems(filteredData);
      } catch (err) {
  ...
  return (
    <div className="max-w-[1200px] mx-auto my-10 px-5 text-[#2c3e50] font-sans w-full">
      <label htmlFor="search" className="text-xl font-bold mb-3 text-center">
        🎯 <strong>Secure your space — trusted home security gear at your fingertips 🏠🚨</strong> 👀 💡
      </label>
      ...
      <section className="mb-12 w-full">
        ...
        ) : homeSecurityItems.length === 0 ? (
          <p className="text-center text-gray-500">
            No home security essentials found.
          </p>
        ) : (
          <div className="flex flex-col gap-4 w-full">
            {homeSecurityItems.map((item) => (
              <div
                key={item.id}
