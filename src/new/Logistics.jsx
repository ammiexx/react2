  ...
  useEffect(() => {
    const fetchLogistics = async () => {
      try {
        ...
        if (!response.ok) throw new Error('Failed to fetch logistic services');
        const data = await response.json();

        const filtered = data.filter(
          (item) => item.category === 'logistic' && item.verified === true
        );
  ...
  return (
    <div className="max-w-[1200px] mx-auto my-10 px-5 text-[#2c3e50] font-sans w-full">
      <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">
        Power up your supply chain — get top logistics and transport materials here 🔧📈
      </h2>
      ...
      <section className="mb-12 w-full">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-500">No logistic services found.</p>
        ) : (
